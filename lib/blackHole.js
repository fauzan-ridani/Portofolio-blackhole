...
export function createBlackHole({ root, canvas, glow, aura }) {
  ...
  function setDiscs() {
    // On tall/narrow viewports (mobile portrait), using the raw container
    // height stretches the ellipses far past a sane aspect ratio — that's
    // what makes the funnel look flat and "seen from above". Cap the height
    // reference relative to width so the disc shape stays consistent across
    // aspect ratios, and only grows the funnel's reach on wide screens.
    const discHeight = Math.min(height, width * 1.35);

    startDisc = { x: width * 0.5, y: discHeight * 0.45, w: width * 0.75, h: discHeight * 0.7 };
    endDisc = { x: width * 0.5, y: discHeight * 0.95, w: 0, h: 0 };

    discs = [];
    let prevBottom = discHeight;
    let clipDisc = null;
